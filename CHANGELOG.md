...
- A request no longer goes into nats-pure's reconnect buffer. While nats-pure reconnects, `publish` buffers up to 32,768 messages, then blocks the calling thread with no deadline (it stayed blocked after the connection closed). After the reconnect, nats-pure sends the whole buffer, so the server ran every retry the caller already saw fail with `RequestTimeout`: a non-idempotent RPC ran up to `max_retries` times. The jnats client turned this buffer off (`:disable_reconnect_buffer`); nats-pure has no such option, and the protection was lost in the migration. `ResponseMuxer#publish` now raises the retryable `Errors::ResponseMuxer` unless the connection is connected, so the client waits `PB_NATS_CLIENT_RECONNECT_DELAY` and retries.
- A boot with NATS down now fails in seconds, not hours. nats-pure's first connect also retries each server `max_reconnect_attempts` times; at the gem default (60,000, 2s apart) `Server#initialize` and the first client request blocked for about 33 hours, and the client held `GET_CONNECTED_MUTEX` all that time. The first connect now uses a budget of 1 (about 4s at nats-pure defaults), then the configured budget applies to every later reconnect. A client that fails to connect raises, and the next request tries again; a server that fails to connect exits, so its supervisor restarts it.
- Callers that wait on `GET_CONNECTED_MUTEX` while a connect fails now fail at once with the new `Errors::ConnectionFailed` (the connect error is its `cause`). Each waiting caller ran its own full connect after the one before it failed, so the Nth caller failed after N connect times (the 8th after 48s with the fleet TLS settings). A caller that arrives after the failure tries again. `ConnectionFailed` is in `RETRYABLE_TRANSPORT_ERRORS`.
